    ...
    for file in sharedFiles:
        try:
            if file in lista_cabeceras:
                ...
                keeptHeaders = pd.read_pickle(
                    backupPath+'backups/'+file.replace('.txt', '')+'.pkl')
                currentHeaders = pd.read_csv(path+'/'+file, sep=';')

                deltaNewHeaders = pd.merge(
                    keeptHeaders, currentHeaders, how='outer', indicator='Control')
                deltaNewHeaders = deltaNewHeaders.loc[deltaNewHeaders['Control']
                                                        == 'right_only']
                deltaNewHeaders = deltaNewHeaders[deltaNewHeaders.columns[:-1]]

                if not (deltaNewHeaders.empty):
                    deltaNewHeaders.to_sql(
                        "NewTedHeaders", sqlite_connection, if_exists='append')
            ...
            if file in lista_detalles:
                ...
                deltaNewDetails = pd.merge(
                    keeptDetails, currentDetails, how='outer', indicator='Control')
                deltaNewDetails = deltaNewDetails.loc[deltaNewDetails['Control']
                                                        == 'right_only']
                deltaNewDetails = deltaNewDetails[deltaNewDetails.columns[:-1]]

                if not (deltaNewDetails.empty):
                    deltaNewDetails.to_sql(
                        "NewTedDetails", sqlite_connection, if_exists='append')
